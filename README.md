...
**Full system architecture diagram:** `ARCHITECTURE.md` (new) — includes `auth`, `banner`, and `category` service flows.

This repository contains three lightweight Serverless Framework services that power user authentication, banner management, and category image uploads for an e-commerce platform. Each service is an isolated AWS Lambda application with its own infrastructure-as-code definition, yet both share a consistent Node.js 24 runtime, HTTP API front door, and DynamoDB persistence for audit trails.
...
| `category/` | Category upload pipeline that returns presigned URLs, stores images in the `category-nimat-images` S3 bucket, and writes metadata into the `Categories` DynamoDB table. |
